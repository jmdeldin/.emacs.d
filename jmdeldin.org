...
use GNU Emacs 24.1+ on Mac OS X and Ubuntu for Org-Mode, Ruby, LaTeX,
sh, R, and a host of miscellaneous other languages.
...
breaking installation. The =custom-file= must be set before trying to
install packages.
...
#+BEGIN_SRC emacs-lisp
  (unless (= (length jm/packages)
             (length (jm/filter 'package-installed-p jm/packages)))
...
#+begin_src emacs-lisp
  (setq auto-save-list-file-prefix (jm/local-path "saves"))
  (setq auto-save-file-name-transforms
        `((".*" ,"~/.emacs.d/local/saves" t)))
#+end_src

Test with =M-x do-auto-save=.
...
#+BEGIN_SRC emacs-lisp
  (mapcar (lambda (mode)
          ...
          '(jabber-chat-mode-hook org-mode-hook markdown-mode git-commit-mode))
#+END_SRC
...
#+begin_src emacs-lisp
  ...
  (setq save-place-file (jm/local-path "/.emacs-places"))
#+end_src
...
#+begin_src emacs-lisp
  (setq projectile-cache-file (jm/local-path "projectile-bookmarks.eld"))
...
#+begin_src emacs-lisp
  (setq bookmark-file (jm/local-path ".emacs.bmk"))
#+end_src
